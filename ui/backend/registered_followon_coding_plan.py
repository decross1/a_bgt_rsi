"""Isolated source-plan reconstruction for one prospective coding study.

The canonical threaded backend keeps the original follow-on loader. This
read-only worker validates the separately registered coding checkout without
changing process-wide imports, calling a model, or reading private responses.
"""
...
MANDATORY_NEW_SOURCE_NAMES = frozenset({
...
def _raw_fingerprint(source: Path, cohort: str) -> tuple[str, ...]:
    ...
    _require(source.parent == grouped.RESEARCH_ROOT /
             ...
             and window.get("study_id") == STUDY_ID
             and window.get("registered_code_root") == str(CODE_ROOT),
             "coding window source/study/root is outside registration")
    bundle = window.get("followon_source_bundle")
    _require(isinstance(bundle, dict)
             and len(bundle) == 42
             and MANDATORY_NEW_SOURCE_NAMES <= set(bundle)
             and grouped._canonical_sha(bundle)
                == window.get("followon_source_bundle_sha256"),
             "registered coding source bundle shape/hash differs")
    module_root = CODE_ROOT / "bench/flash_next_ab"
    source_fingerprints = []
    for name in sorted(bundle):
        _require(isinstance(name, str) and name == Path(name).name
                 and len(name) <= 128,
                 "registered coding module name is malformed")
        ref = bundle[name]
        expected_path = module_root / name
        _require(isinstance(ref, dict)
                 and set(ref) == {"path", "sha256", "bytes"}
                 and ref["path"] == str(expected_path)
                 and type(ref["bytes"]) is int
                 and 0 < ref["bytes"] <= MAX_SOURCE_BYTES,
                 "registered coding module path/size differs")
        raw = mr._read_path(expected_path, maximum=MAX_SOURCE_BYTES,
                            label=f"registered coding module {name}")
        _require(len(raw) == ref["bytes"]
                 and mr._sha256(raw) == ref["sha256"],
                 "registered coding module raw bytes changed")
        source_fingerprints.append(ref["sha256"])
...
def registered_expected(source: Path, output: Path,
    ...
    _require(cohort == "flash", "coding validator only accepts Flash")
