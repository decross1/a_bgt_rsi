import { registeredFlashVariants } from "../types/registeredFlashVariants";
...
function registeredMiaVariant(value: unknown): value is Variant {
  ...
  const registered = typeof row.id === "string" && Object.hasOwn(registeredFlashVariants, row.id)
    ? registeredFlashVariants[row.id] : undefined;
  return !!registered &&
    row.spec_sha256 === registered.spec_sha256 &&
    ...
    row.image_id === registered.image_id &&
    row.model_artifact_sha256 === "a40ce50173dd3aff54da88503894967e5248bbb927f9e4a91eff5a6a7270c168" &&
    row.qualification_profile === registered.qualification_profile &&
  ...
  return <section className="research-pipeline local-model-research" aria-labelledby="local-model-research-heading">
    ...
    {data.status === "unavailable" ? <p className="benchmark-empty-inline">Local evaluation receipts are unavailable. No progress or result is inferred.</p> : <>
      ...
      {qualifications.length === 0 ? <p className="benchmark-empty-inline">No qualification result recorded yet. Downloading the model does not establish that it runs reliably.</p> : <div className="benchmark-table-wrap" role="region" tabIndex={0} aria-label="Runtime qualification table, scroll horizontally"><table className="benchmark-table">
        ...
        <thead><tr><th scope="col">Run</th><th scope="col">Variant</th><th scope="col">Result</th><th scope="col">Candidate window</th><th scope="col">Server startup</th><th scope="col">Host memory headroom</th><th scope="col">Probes</th><th scope="col">Resident restoration</th></tr></thead>
        <tbody>{qualifications.map(run => <tr key={run.id}>
          ...
          <td>{registeredMiaVariant(run.variant) ? <><strong>Mia NVFP4</strong><small>{run.variant.revision.slice(0, 8)} · {run.variant.evidence_class === "QUALIFICATION_ADMITTED" ? "runtime qualified" : "registered source only"}</small><details className="benchmark-provenance"><summary>Variant provenance</summary><dl><dt>Runtime profile</dt><dd>{run.variant.qualification_profile}</dd><dt>Context / MTP</dt><dd>{registeredFlashVariants[run.variant.id].max_model_len.toLocaleString()} total tokens · MTP {registeredFlashVariants[run.variant.id].mtp_speculative_tokens}</dd><dt>KV allocation</dt><dd>{(registeredFlashVariants[run.variant.id].kv_cache_memory_bytes / 2 ** 30).toFixed(1)} GiB</dd><dt>Repository</dt><dd>{run.variant.repository}</dd><dt>Revision</dt><dd>{run.variant.revision}</dd><dt>Image SHA256</dt><dd>{run.variant.image_id}</dd><dt>Artifact SHA256</dt><dd>{run.variant.model_artifact_sha256}</dd><dt>Spec SHA256</dt><dd>{run.variant.spec_sha256}</dd></dl></details></> : run.id.startsWith("qfn-c0-") ? run.status === "unfinished_receipt" ? "Model identity pending" : "NVIDIA NVFP4 · recorded receipt" : "Variant unavailable"}</td>
          <td>{run.status === "unfinished_receipt" ? "Unfinished receipt · process state unverified" : run.failure_class === "experimental_startup_host_pageout_guardrail_abort" ? "Host paging guardrail stopped qualification · no model-quality conclusion" : run.status === "failed" && run.model_started === false ? "Setup failed · model not started" : label(run.status)}{typeof run.qualification_elapsed_seconds === "number" && Number.isFinite(run.qualification_elapsed_seconds) && run.qualification_elapsed_seconds >= 0 && <small>Full qualification: {number(run.qualification_elapsed_seconds)} s</small>}</td>
          <td>{run.model_started === false ? "Not started" : run.candidate_window_minutes === null ? "Not recorded" : `${number(run.candidate_window_minutes)} min maximum`}</td>
          <td>{run.startup_status === "recorded" && typeof run.startup_seconds === "number" && Number.isFinite(run.startup_seconds) && run.startup_seconds > 0
            ? <>{number(run.startup_seconds)} s<small>Container start → model endpoint ready</small>{typeof run.startup_source_sha256 === "string" && /^[0-9a-f]{64}$/.test(run.startup_source_sha256) && <details className="benchmark-provenance"><summary>Startup sources</summary><dl><dt>Readiness SHA256</dt><dd>{run.startup_source_sha256}</dd>{typeof run.source_sha256 === "string" && /^[0-9a-f]{64}$/.test(run.source_sha256) && <><dt>Result SHA256</dt><dd>{run.source_sha256}</dd></>}</dl></details>}</>
            : run.startup_status === "unavailable" ? "Source unavailable" : "Not recorded"}</td>
          <td>{run.minimum_memory_gib === null ? "Not recorded" : `${number(run.minimum_memory_gib)} GiB minimum`}</td>
          <td>{number(run.probe_count, 0)}</td><td>{label(run.restoration)}</td>
        </tr>)}</tbody>
      </table></div>}
      {qualifications.length > 0 && <p className="benchmark-empty-inline">Server startup measures container start to the first valid model-endpoint response. It is separate from the full qualification window and from request latency. Candidate windows measure elapsed time, not GPU utilization. Host memory headroom includes setup and restoration.</p>}
      ...
      {comparisons.length === 0 ? <p className="benchmark-empty-inline">No validated paired run recorded yet. A runtime qualification pass is separate from a capability improvement.</p> : comparisons.map(comparison => <section key={comparison.id} aria-label={`Comparison ${comparison.id}`}>
        ...
        {comparison.admission_class === "RECORDED_COMPLETED_PAIR_ADMISSION" && comparison.comparison_eligible_at_recording === true && <p>
          <span className="benchmark-chip benchmark-chip--info">Completed pair admitted</span>{" "}
          {comparison.current_source_replay === "verified"
            ? "Recorded evidence also passes source replay now."
            : "Recorded evidence remains bound to its original hashes; source replay is currently unavailable."}
        </p>}
        {(comparison.status !== "complete" || comparison.comparison_eligible !== true || !admittedPair(comparison)) && <p>Comparison incomplete or without verified recorded admission. Counts show recorded attempts; rates and gains are withheld.</p>}
        <div className="benchmark-table-wrap" role="region" tabIndex={0} aria-label={`Paired results for ${comparison.id}, scroll horizontally`}><table className="benchmark-table">
          ...
          <tbody>{comparison.families.map(family => {
            ...
            const eligible = comparison.status === "complete" && comparison.comparison_eligible === true && admittedPair(comparison) && family.comparison_eligible === true;
