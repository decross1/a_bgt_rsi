...
const stamp = (value: unknown): value is string => text(value, 64) && Number.isFinite(Date.parse(value));
...
function plan(value: unknown, revision: unknown): V3Plan | undefined {
  ...
  if (!record(value) || !text(value.id, 64) || value.id !== revision || !text(value.revision, 12) ||
      !text(value.path, 200) || !stamp(value.written_at) || typeof value.is_current !== "boolean") return undefined;
...
function focus(value: unknown): V3Focus | null {
  ...
  if (title === undefined || stage === undefined || nextAction === undefined) return null;
...
function work(value: unknown): V3Work[] | null {
  ...
  for (const item of value) {
    if (!record(item) || !text(item.id, 40) || !text(item.goal, 300) || !text(item.owner, 300) ||
    ...
    if (whyToday === undefined || acceptance === undefined || dependsOn === null) return null;
...
function waiting(value: unknown): V3Waiting[] | null {
  ...
  for (const item of value) {
    if (!record(item) || !["question", "owner_decision"].includes(String(item.kind)) || !text(item.id, 120) ||
    ...
    if (context === undefined || recommendation === undefined || consequence === undefined || askedAt === undefined ||
        msgId === undefined || handoffMsgId === undefined || choices === null ||
...
function updates(value: unknown): V3Update[] | null {
  ...
  const rows: V3Update[] = [];
  for (const item of value) {
    if (!record(item) || !text(item.id, 80) || !text(item.title, 300) || !text(item.disposition, 40) ||
    ...
    rows.push({ id: item.id, title: item.title, disposition: item.disposition, summary: item.summary,
...
function achievements(value: unknown, limit: number): V3Achievement[] | null {
  ...
  for (const item of value) {
    if (!record(item) || !text(item.id, 120) || !text(item.kind, 80) || !text(item.title, 400) || !stamp(item.at) || !text(item.evidence, 240)) return null;
    rows.push({ id: item.id, kind: item.kind, title: item.title, at: item.at, evidence: item.evidence });
  }
  return rows;
}
...
function improvements(value: unknown): V3Achievement[] | null {
  ...
  const rows: V3Achievement[] = [];
  for (const item of value) {
    if (!record(item) || !text(item.sha, 40) || !stamp(item.at) || !text(item.subject, 240) ||
        strings(item.goals, 6, 12) === null) return null;
    rows.push({ id: item.sha, kind: "improvement", title: item.subject, at: item.at, evidence: item.sha });
  }
  return rows;
}
...
function agents(value: unknown): V3Agent[] | null {
  ...
  for (const key of ["oracle", "pi_client", "nara", "meta_oracle"]) {
    ...
    if (!record(item) || !text(item.label, 512) || !text(item.status, 32) || !text(item.detail, 4096) ||
        !stamp(item.observed_at) || !text(item.source, 512)) return null;
    const role = optionalText(item.role, 512);
    const activity = optionalText(item.activity, 512);
    if (role === undefined || activity === undefined) return null;
    rows.push({ label: item.label, role, status: item.status, detail: item.detail, observedAt: item.observed_at,
...
export function admitDailyOpsV3Summary(value: unknown): DailyOpsV3Summary | null {
  if (!record(value) || value.schema_version !== "daily-ops-summary/v3" || !stamp(value.generated_at) ||
  ...
  if (parsedPlan === undefined || parsedFocus === null || parsedWork === null || parsedWaiting === null ||
      ...
      parsedAgents === null || warnings === null) return null;
