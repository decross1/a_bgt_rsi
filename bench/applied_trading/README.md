...
```bash
.venv-chroma/bin/python -m bench.applied_trading.public_spot_capture --plan --symbol BTCUSDT --max-pages 32
.venv-chroma/bin/python -m bench.applied_trading.public_spot_capture --run --symbol BTCUSDT --max-pages 1 --output-dir /home/decross1/projects/a_bgt_rsi_v2_artifacts/2026-09-15/applied-trading-public-data/captures/spot-BTCUSDT-YYYYMMDDTHHMMSSZ
...
.venv-chroma/bin/python -m bench.applied_trading.public_spot_capture --run --symbol BTCUSDT --from-id ACTUAL_SEALED_CURSOR --max-pages 32 --output-dir /home/decross1/projects/a_bgt_rsi_v2_artifacts/2026-09-15/applied-trading-public-data/captures/spot-BTCUSDT-YYYYMMDDTHHMMSSZ
.venv-chroma/bin/python -m bench.applied_trading.hourly_capture --plan
.venv-chroma/bin/python -m bench.applied_trading.hourly_capture --run-once --previous-batch ABSOLUTE_COMPLETE_BATCH --bootstrap-warmup ABSOLUTE_FIRST_WARMUP --max-pages 32 --output-dir /home/decross1/projects/a_bgt_rsi_v2_artifacts/2026-09-15/applied-trading-public-data/captures/spot-BTCUSDT-YYYYMMDDTHHMMSSZ
```

`hourly_capture` is a **manual one-shot** locked continuation. For later invocations omit `--bootstrap-warmup`, and give its last `continued_complete` batch as predecessor. At most two metadata GETs and 32 trade-page GETs are issued under the registered cap. It stops if an attempt fails, a cursor/page gap appears, or the 32-page cap cannot catch up. An incomplete continuation remains an immutable failed attempt; it cannot donate a later cursor. A source-reviewed five-minute [capture tick](capture_tick.py) and [systemd user units](systemd/applied-h1-capture.timer) are packaged for after the model decision. They journal zero-GET research-lease skips, source-bound state and every capture denominator. A verified cursor older than 15 minutes archives its old state and explicit missing interval before starting a **new** venue-tail lineage; failed, interrupted or source-drift branches remain blocked. Dated activation receipts establish whether the timer is running; the package itself issues no GET.
...
After genuinely complete, cursor-contiguous batches span a **closed** hour, `prospective_hourly_features` can emit as-of features with source-frame hashes and availability times. A stale book or missing left/right trade marker withholds a decision. The first fixed **unfitted engineering** pilot is a separately typed [REST displayed-quote diagnostic](REST_REFERENCE_OPERATOR.md), bound to the [known-prior H1 transfer](H1_STRATEGIC_TRANSFER.md). Its selected ask/bid requests start after decision/hold boundaries, but REST L5 has no depth-event sequence or venue event timestamp. It always reports `paper_supported=false` and cannot be called an executable fill. The later strict `applied_trial/v1` and `paper_driver` require externally admitted sequence-valid observation/quote proof, full scheduled denominator and doubled-cost sensitivity before a paper-supported outcome. There is no automatic science L1–L5 promotion or order path. The 30-day sequence is in [30_DAY_EXECUTION.md](30_DAY_EXECUTION.md), with exact [60-day and timer operations](OPERATIONS_60_DAY_AND_TIMER.md).
