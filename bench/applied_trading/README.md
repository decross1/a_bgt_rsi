...
`hourly_capture` is a **manual one-shot** locked continuation. For later invocations omit `--bootstrap-warmup`, and give its last `continued_complete` batch as predecessor. At most two metadata GETs and eight trade-page GETs are issued. It stops if an attempt fails, a cursor/page gap appears, or the eight-page cap cannot catch up. An incomplete continuation remains an immutable failed attempt; it cannot donate a later cursor. A source-reviewed five-minute [capture tick](capture_tick.py) and disabled [systemd user units](systemd/applied-h1-capture.timer) are packaged for after the model decision. They journal zero-GET research-lease skips, source-bound state and every capture denominator. A verified cursor older than 15 minutes archives its old state and explicit missing interval before starting a **new** venue-tail lineage; failed, interrupted or source-drift branches remain blocked. Packaging does not enable the timer or issue a GET.
