...
After capture has continued through at least `forward_end + hold_s + exit_timeout_s`, derive `features.jsonl`/`feature-source.json` using the `prospective_hourly_features` CLI on a contiguous ordered list of at most 96 sealed **complete_incremental_batch** directories and at most 500,000 trade events. At a two-minute capture cadence, 96 batches span only about 3h12m; even a 2h block plus the fixed 90m lookback and 70m hold/exit reserve can exceed this unchanged bound. A busy trade period may hit the 500,000-event bound sooner. The current producer cannot validate an over-cap block by dropping intervening cursor-linked batches. A future forward study needs separately frozen source/caps or a correctly registered shorter observation design, and must keep every failed/missing block cell in its denominator; this script makes no 30-day claim. Then run from the installed checkout; pass the *same* ordered batch list used to build features:
