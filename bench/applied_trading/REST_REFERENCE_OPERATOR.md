...
After capture has continued through at least `forward_end + hold_s + exit_timeout_s`, derive `features.jsonl`/`feature-source.json` using the `prospective_hourly_features` CLI on a contiguous ordered list of at most 96 sealed **complete_incremental_batch** directories and at most 500,000 trade events. A nominal 2h block with the fixed 90m lookback and 70m hold/exit reserve spans 280m, about 70 four-minute ticks plus boundary/late-source allowance, within 96 when capture remains on that cadence. A busy trade period may hit the 500,000-event bound sooner. The current producer cannot validate an over-cap block by dropping intervening cursor-linked batches, and a later 2-minute cadence would exceed 96 for the same horizon. A future forward study must freeze its observed source/cadence and keep every failed or missing block cell in its denominator; this script makes no 30-day claim. Then run from the installed checkout; pass the *same* ordered batch list used to build features:
