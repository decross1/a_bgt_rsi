"""Run matched 8K/16K/32K TOTAL-context diagnostics under a lab controller."""
...
PLAN_SCHEMA = 'lab-model-context-plan/v1'
RUN_SCHEMA = 'lab-model-context-run/v1'
...
def build_plan() -> tuple[dict[str, Any], dict[str, dict]]:
    ...
    selected = ('resident_gemma', 'flash_next_mia')
    endpoints = {
        name: {**identities[name], 'policies': {POLICY_ID: _policy(name)}}
        for name in selected
    }
    rows = {row['cell_id']: row for row in packet['cells']}
    declared = [row['cell_id'] for row in packet['cells']]
    plan = {
        ...
        'endpoints': endpoints,
        'call_routes': {
            'resident': {cell_id: ['resident_gemma'] for cell_id in declared},
...
def freeze_plan(path: str | Path) -> dict[str, Any]:
    ...
    return {'path': str(destination), 'raw_sha256': hashlib.sha256(raw).hexdigest(),
            'plan_sha256': manifest.sha256_json(plan), 'declared_cells': 36}
...
def run(
    ...
    if (cohort not in COHORTS or not callable(admission_gate)
            ...
            or not math.isfinite(runtime_budget_s) or not 0 < runtime_budget_s <= 6000):
    ...
    outcomes = []
    ordinal = 0
    endpoint_name = 'resident_gemma' if cohort == 'resident' else 'flash_next_mia'
    ...
    result = {
        ...
        'candidate_variant_id': plan['runtime_certificates']['flash']['candidate_spec_id']
                                if cohort == 'flash' else 'resident-gemma',
...
def replay_run(plan_path: str | Path, run_path: str | Path) -> dict[str, Any]:
    """Recheck private SSE and unchanged objective/citation grades for all 36."""
    plan, rows, plan_raw_sha = load_plan(plan_path)
    raw, actual = harness._read_regular_file(
    ...
    return {
        'schema_version': 'lab-model-context-grade-replay/v1',
        ...
        'primary_replay_passed': len(run['outcomes']) == 36 and not mismatches,
