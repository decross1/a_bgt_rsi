...
def test_pinned_bundle_serves_c4_m20_with_exact_context_and_minimum_realized_pool():
    import hashlib
    import re
    import pytest
    if not resident.BUNDLE.exists():
    ...
    assert profile_name == 'nextn-262k-c4-m20-min224-s3.json'
    ...
    # permits for four running requests. Launch-time unified-memory variation,
    # not this M20 setting, is the leading explanation for observed pool size.
    ...
    assert 'MIN_READY_TOTAL_TOKENS = 224 * 1024' in text
    assert 'type(observed_pool) is not int' in readiness
    assert 'observed_pool < MIN_READY_TOTAL_TOKENS' in readiness
    assert '{"minimum": MIN_READY_TOTAL_TOKENS, "requested": 262144}' in readiness
...
def test_selected_rejects_previous_and_unreviewed_profile_digests(tmp_path):
    ...
    for digest in (
        ...
        '01436b7e2c515f2414b1693675d07a1de4504728455adc94f1e15da45c31007d',
    ):
        with pytest.raises(ValueError, match='reviewed serving bundle'):
