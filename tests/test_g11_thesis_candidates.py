"""Acceptance for plan 2026-09-24 d1 rev2 (G1.1): 3-5 thesis candidates, SHAPE only
(brief lines 39-53), never prose; citation reality is Oracle's screen (D-061).
Reads `### Field`, `**Field:**` and `| Field |` shapes; fixture holds one each.
"""
import re
...
FIELDS = {"title": ("title",), "question": ("question",), "lens": ("lens",),
          ...
          "conviction": ("conviction",), "builds": ("builds on",)}
REQUIRED = tuple(FIELDS)  # present and filled in every candidate
STAGES = {'theory': [(r"openspiel|decision problem|\bgam", "a problem"), (r"benchmark|equilibrium", "a benchmark"), (r"decision rule|preregist|threshold", "a rule")],
 'semi': [(r"arm", "arms"), (r"n\s*=\s*\d|sample", "a sample"), (r"outcome", "an outcome"), (r"missing", "a missing rule")],
 'applied': [(r"option|prediction market|crypto|market|venue", "a market"), (r"\bdata\b|snapshot|historical|series|history", "its data")]}
...
def test_every_field_is_filled_and_the_line_is_honest():
    for rng in ranges():
        ...
        assert re.search(r"iter-\d{4}-\d\d-\d\d-\d{3}", field(rng, "builds")), "Builds on names no iteration id"
...
def test_lens_is_one_two_or_three_and_two_carries_the_rigor_rule():
    for rng in ranges():
        ...
        if "2" in used:  # on the Lens field
