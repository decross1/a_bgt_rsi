...
from .frontier_cli import CODEX_MODEL, CODEX_REASONING_EFFORT
from .frontier_cli import _parse_claude_stdout, _parse_codex_stdout
...
def _run(cmd, *, env, cwd, deadline, input_text=""):
...
def _preflight(vendor, env, cwd, deadline):
...
def invoke_maintenance_frontier(vendor, prompt, *, timeout_s, role, ledger_path):
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="weekly-frontier-") as tmp:
            ...
            binary, version, failures = _preflight(vendor, env, tmp, deadline)
            result.update(resolved_binary=binary, cli_version=version, binary_fallbacks=failures)
            category = "AUTH_FAILED"
            if vendor == "claude":
                auth = _run([binary, "auth", "status"], env=env, cwd=tmp, deadline=deadline)
