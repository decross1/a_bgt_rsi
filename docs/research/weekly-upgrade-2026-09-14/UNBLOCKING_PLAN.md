...
does not coordinate a GPU lease with the production coordinator. A short live
canary can proceed when idle; unattended long runs need resource coordination
implemented before activation. Do not stop a production service merely to
complete this pilot.

## Convert weekly advice into executed work

Extend the existing controller and evaluator with a typed trial dispatcher;
avoid a second independent weekly system. An analyst proposal identifies a
supported experiment and parameter values. Repository code validates those
values and constructs the command. Free-form analyst shell commands are never
an execution interface.

Each dispatched trial needs an immutable input manifest, a budget reservation,
a unique experiment ID, isolated outputs, a cooperative resource lock, a hard
subprocess deadline, and a durable terminal status. Resume reads the journal;
...
tested and delivered under standing maintenance authority. A proposed serving
change needs a concrete configuration, evidence and rollback before the final
activation decision. That boundary comes from the owner's standing authority
and the handoff's explicit exclusion of a production cutover, not a new Git
